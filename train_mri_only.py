...
from models.mri_survnet import MRIOnlySurvNet
...
def train_one_fold(fold_id, args, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"[MRI] Fold {fold_id} - Epoch {epoch} [train]"):
            mri = batch["mri"].to(device)    # only MRI
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"[MRI] Fold {fold_id} - Epoch {epoch} [val]"):
        ...
        print(
            f"[MRI Fold {fold_id}] Epoch {epoch} | "
        ...
        if c_index > best_cindex:
            ...
            path = os.path.join(model_dir, f"mri_only_survnet_fold{fold_id}.pth")
            torch.save(best_state, path)
            print(f"Saved best MRI-ONLY model for fold {fold_id}: {c_index:.4f} to {path}")

    # --- save per-case predictions ---
    if len(all_val_preds) > 0:
        preds_df = pd.DataFrame(all_val_preds)
        os.makedirs(args.out_dir, exist_ok=True)
        csv_path = os.path.join(args.out_dir, f"predictions_mri_fold{fold_id}.csv")
        preds_df.to_csv(csv_path, index=False)
        print(f"Saved MRI-only fold {fold_id} predictions to {csv_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=30)    # match main config if you want
    ...
    parser.add_argument("--out_dir", type=str, default="outputs_mri")
